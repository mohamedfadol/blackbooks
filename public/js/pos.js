$(document).ready(function() {
    ...
    $('table#pos_table tbody').on('change', 'input.pos_quantity', function() {
        ...
        $('span.custom_tex_total').html(__currency_trans_from_en((line_total- unit_price_inc_tax), false));
    ...
    //If change in unit price update price including tax and line total tttttttt1
    $('table#pos_table tbody').on('change', 'input.pos_unit_price', function() {
        ...
        $('span.custom_tex_total').html(__currency_trans_from_en(line_total, false));
    });
    ...
    $('table#pos_table tbody').on('change', 'input.pos_unit_price_inc_tax', function() {
        ...
        __write_number(tr.find('input.pos_unit_price'), unit_price);
        __write_number(tr.find('input.pos_line_total'), line_total, false, 2);
        $('span.custom_tex_total').html(__currency_trans_from_en(600000, false));
        calculateCustomTax(line_total , unit_price);
        tr.find('span.pos_line_total_text').text(__currency_trans_from_en(line_total, true));
    ...
    $('table#pos_table tbody').on(
        ...
        function() {
            ...
            var unit_price_inc_tax = __add_percent(discounted_unit_price, tax_rate);
            var line_total = quantity * unit_price_inc_tax;
            $('span.custom_tex_total').html(__currency_trans_from_en((unit_price_inc_tax), false));
            __write_number(tr.find('input.pos_unit_price_inc_tax'), unit_price_inc_tax);
            __write_number(tr.find('input.pos_line_total'), line_total, false, 2);
            tr.find('span.pos_line_total_text').text(__currency_trans_from_en(line_total, true));
            pos_each_row(tr);
        ...
        $.ajax({
            ...
            success: function(result) {
                if (result.success) {
                    ...
                    pos_total_row();
                    // $('span.custom_tex_total').html(__currency_trans_from_en(99000000000, false));
...
function pos_each_row(row_obj) {
    ...
    $('span.custom_tex_total').html(__currency_trans_from_en((line_total - unit_price), false));
...
function get_subtotal() {
    ...
    $('input.modifiers_price').each(function() {
        ...
        calculateCustomTax(price_total , modifier_subtotal);
    });
    
    return price_total;
}

function calculateCustomTax(pos_line_total , pos_unit_price){
    var customTax = pos_line_total - pos_unit_price; 
    return $('span.custom_tex_total').html(__currency_trans_from_en(customTax, false));
...
$('table#pos_table tbody').on('change', 'input.pos_line_total', function() {
    ...
    $('span.custom_tex_total').html(__currency_trans_from_en(5000000000, false));
    pos_total_row();
});
...
function adjustComboQty(tr){
    if(tr.find('input.product_type').val() == 'combo'){
        ...
        $('span.custom_tex_total').html(__currency_trans_from_en(3000000000, false));
        ...
        success: function(result) {
            if (result.html) {
                ...
                $(html).find('tr').each(function(){
                    ...
                    $('span.custom_tex_total').html(__currency_trans_from_en(6000000000, false));
                    //For initial discount if present
                    var line_total = __read_number(this_row.find('input.pos_line_total'));
                    this_row.find('span.pos_line_total_text').text(line_total);
