...
class RestaurantUtil extends Util
{
    ...
    public function getAllReceivedOrders($business_id, $filter = [])
    {
        $query = Transaction::leftJoin('contacts', 'transactions.contact_id', '=', 'contacts.id')
                ->leftjoin(
     ...
     * @param  int  $business_id
     * @param  array  $filter
     * *For new orders order_status is 'cooked'
     * @return obj $orders
     */
    public function getAllOrdersCooked($business_id, $filter = [])
    {
        $query = Transaction::leftJoin('contacts', 'transactions.contact_id', '=', 'contacts.id')
                ->leftjoin(
                    'business_locations AS bl',
     ...
     * @return obj $orders
     */
    public function getAllOrdersRerved($business_id, $filter = [])
    {
        $query = Transaction::leftJoin('contacts', 'transactions.contact_id', '=', 'contacts.id')
                ->leftjoin(
                    'business_locations AS bl',
     ...
     * @return obj $orders
     */
    public function getAllOrdersReturnsKitchen($business_id, $filter = [])
    {
        $query = Transaction::leftJoin('contacts', 'transactions.contact_id', '=', 'contacts.id')
                ->leftjoin(
                    'business_locations AS bl',
        ...
        if (! empty($filter['line_order_status'])) {
            if ($filter['line_order_status'] == 'returns') {
                $query->whereHas('sell_lines', function ($q) {
                    $q->where('res_line_order_status', 'returned');
                }, '>=', 1);
            }
        }
        $permitted_locations = auth()->user()->permitted_locations();
