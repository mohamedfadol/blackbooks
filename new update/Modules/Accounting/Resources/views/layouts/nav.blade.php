            ...
            <div class="collapse navbar-collapse" id="bs-example-navbar-collapse-1">
                <ul class="nav navbar-nav">
                    ...
                    @if(auth()->user()->can('accounting.view_journal'))
                        ...
                        <a href="{{action([\Modules\Accounting\Http\Controllers\JournalEntryController::class, 'journalOpenning'], ['type' => 'journal_openning'])}}">@lang('accounting::lang.journal_entry_2')</a></li>
                    @endif

                    @if(auth()->user()->can('accounting.view_journal'))
                        <li @if(request()->segment(2) == 'journal-entry' && request()->get('type') == 'journal_company_general') class="active" @endif>
                        <a href="{{action([\Modules\Accounting\Http\Controllers\JournalEntryController::class, 'journalCompanyGeneral'], ['type' => 'journal_company_general'])}}">@lang('accounting::lang.journal_entry_3')</a></li>
                    @endif
