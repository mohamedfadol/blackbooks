...
class JournalEntryController extends Controller
{
     ...
     * @return Response
     */
    public function journalCompanyGeneral()
    {
        $business_id = request()->session()->get('user.business_id');
        ...
        if (request()->ajax()) { 
            $journal = AccountingAccTransMapping::where('accounting_acc_trans_mappings.business_id', $business_id)
                        ->join('users as u', 'accounting_acc_trans_mappings.created_by', 'u.id')
                        ->where('journal_company_general', 'journal_company_general')
                        ->select(['accounting_acc_trans_mappings.id', 'ref_no', 'operation_date',  'number', 'original', 'note',
                            DB::raw("CONCAT(COALESCE(u.surname, ''),' ',COALESCE(u.first_name, ''),' ',COALESCE(u.last_name,'')) as added_by"),
                        ]);
        ...
        return view('accounting::journal_entry.journal_company_general');
    }
     ...
     * @return Response
     */
    public function journalOpenning()
    {
        $business_id = request()->session()->get('user.business_id');
        ...
        if (request()->ajax()) { 
            $journal = AccountingAccTransMapping::where('accounting_acc_trans_mappings.business_id', $business_id)
                        ->join('users as u', 'accounting_acc_trans_mappings.created_by', 'u.id')
                        ->where('journal_openning', 'journal_openning')
            ...
            if (! empty(request()->start_date) && ! empty(request()->end_date)) {
                $start = request()->start_date;
                $end = request()->end_date;
                $journal->where('accounting_acc_trans_mappings.type', '!=','journal_entry');
                $journal->whereDate('accounting_acc_trans_mappings.operation_date', '>=', $start)
                            ->whereDate('accounting_acc_trans_mappings.operation_date', '<=', $end);
            }
        ...
        return view('accounting::journal_entry.journal_openning');
    }
